...
public class Piano {
  ...
  static int width(int octaves) {
    return MARGIN + (octaves * 7) * (WHITE_KEY_WIDTH + MARGIN) + MARGIN;
  }
  ...
  static Canvas create(int width, int height, int octaves) {
    ...
    for (int i = 0; i < whiteKeys; ++i) {
      var x = MARGIN + i * (WHITE_KEY_WIDTH + MARGIN);
      var y = MARGIN;
    ...
    for (int i = 0; i < octaves; ++i) {
      for (int offset = 0; offset < blackKeyOffsets.length; ++offset) {
        var x = MARGIN + (i * (WHITE_KEY_WIDTH + MARGIN) * 7) + (blackKeyOffsets[offset] * (WHITE_KEY_WIDTH + MARGIN))
